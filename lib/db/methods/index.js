...
const createWithAddress = async (Model, obj, address) => {
  ...
  if (model === null) {
    ...
    console.log(logSymbols.success, `${modelName} ${newModel.id} saved to database`)
  }
...
const updateModel = async (Model, obj, lookby = {}, updateMsg = 'updated') => {
  ...
  if (affectedCount > 0) {
    console.log(logSymbols.success, `${Model.name} ${obj.id} ${updateMsg}`)
  } else {
    console.log(logSymbols.error, `${Model.name} with ID ${obj.id} not updated`)
  }
}
