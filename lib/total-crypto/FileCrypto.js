...
class FileCrypto {
  ...
  constructor (key, nonce) {
    ...
    nonce.buffer.copy(this.encryptionNonce, 0, 0, BUFFER_OFFSET)
    nonce.buffer.copy(this.decryptionNonce, 0, 0, BUFFER_OFFSET)
  }
  ...
  encryptChunk (blockDataBuffer, hmacKey) {
    return new Promise((resolve, reject) => {
      ...
      const cipher = crypto.createCipheriv(algorithm, this.key.buffer, this.encryptionNonce)

      // encrypt the data
      const cipherText = Buffer.concat([cipher.update(blockDataBuffer), cipher.final()])

      // THEN MAC
      const hmac = crypto.createHmac(macFunction, hmacKey.buffer)
  ...
  decryptChunk (cipherbBlockDataBuffer, macBuffer, hmacKey, counter) {
    return new Promise((resolve, reject) => {
      ...
      const cipher = crypto.createDecipheriv(algorithm, this.key.buffer, this.decryptionNonce)
