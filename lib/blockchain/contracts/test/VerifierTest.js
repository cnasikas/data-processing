...
contract('Verifier', (accounts) => {
  before(async () => {
    const data = await readFile(path.join(__dirname, 'data'))
  ...
  describe('Proof verification', () => {
    ...
    it('shouldn not verify incorrect proof', async () => {
      ...
      const wrongProof = await this.verifier.verifyTx.call([0, 0], AH, BG, BH, CG, CH, H, K, [35])
      const wrongInput = await this.verifier.verifyTx.call(AG, AH, BG, BH, CG, CH, H, K, [0])
