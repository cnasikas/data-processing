...
contract('DataStore', (accounts) => {
  ...
  describe('not the contract owner', () => {
    it('cannot register data processor', async () => {
      try {
        await registerEntity(this.dataStore, web3, processorEntityInfo, { from: accounts[4] })
    ...
    it('cannot register data controller', async () => {
      try {
        await registerEntity(this.dataStore, web3, controllerEntityInfo, { from: accounts[4] })
        assert.isTrue(false) // should never reach here
      } catch (error) {
  ...
  describe('not a registered entity', () => {
    it('cannot register dataset', async () => {
      try {
        ...
        await registerDataset(this.dataStore, web3, { ...entries.dataset }, accounts[4])
        assert.isTrue(false) // should never reach here
      } catch (error) {
  ...
  describe('cannot request processing', () => {
    it('dataset not exists', async () => {
      ...
      try {
        ...
        await requestProcessing(this.dataStore, web3, { ...datasetOptions }, { ...entries.request }, requestorAccount)
        assert.isTrue(false) // should never reach here
      } catch (error) {
        const algorithmID = padRight(web3.fromAscii(entries.request.algorithm), 64)
        const request = await this.dataStore.requests.call(soliditySha3(datasetOptions.hash, requestorAccount, algorithmID))
        request[request.length - 1].should.be.false
      }
    })
    ...
    it('request already exists', async () => {
      try {
        ...
        const algorithmID = padRight(web3.fromAscii(entries.request.algorithm), 64)
        const request = await this.dataStore.requests.call(soliditySha3(entries.dataset.hash, requestorAccount, algorithmID))
        request[request.length - 1].should.be.true
      }
    })
  })
  ...
  describe('cannot notify processor', () => {
    it('request does not exists', async () => {
      ...
      try {
        ...
        request[5].should.be.equal(EMPTY_ADDRESS)
      }
    })
