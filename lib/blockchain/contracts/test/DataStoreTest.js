...
contract('DataStore', (accounts) => {
  ...
  describe('dataset processing', () => {
    it('should request a data processing', async () => {
      ...
      const watcher = async (err, result) => {
        ...
        const request = await this.dataStore.requests.call(_requestID)

        request[0].should.equal(entries.dataset.hash)
        request[1].should.equal(requestorAccount)
        should.equal(trimNullBytes(web3.toAscii(request[3])), entries.request.algorithm)
        request[4].should.equal(entries.request.pubKey)
      }

      await awaitEvent(event, watcher)
    })

    it('should request a data processing for a different algorithm', async () => {
      await registerEntity(this.dataStore, web3, processorEntityInfo)
      await registerEntity(this.dataStore, web3, controllerEntityInfo)
      await registerDataset(this.dataStore, web3, { ...entries.dataset }, controllerAccount)
      await requestProcessing(this.dataStore, web3, { ...entries.dataset }, { ...entries.request }, requestorAccount)

      const newRequest = { ...entries.request }
      newRequest.algorithm = 'min'
      await requestProcessing(this.dataStore, web3, { ...entries.dataset }, { ...newRequest }, requestorAccount)
    })
    ...
    it('should notify a data processor for data processing', async () => {
      ...
      await this.dataStore.notifyProcessor(_requestID, JSON.stringify(cipher), { from: controllerAccount })
      ...
      const watcher = async (err, result) => {
        ...
        request[5].should.equal(processorAccount)
        request[2].should.be.false
      }
  ...
  describe('cannot notify processor', () => {
    it('request does not exists', async () => {
      ...
      shouldFail.reverting(this.dataStore.notifyProcessor(requestID, JSON.stringify(cipher)))
    })
    ...
    it('controller is not the owner of the dataset', async () => {
      ...
      shouldFail.reverting(this.dataStore.notifyProcessor(requestID, JSON.stringify(cipher), { from: accounts[5] }))
    })
  })
})
