...
const { parseProof, normalizeProof } = require('data-market-utils')
...
const saveProof = async (requestID, account) => {
  const tx = await this.proofStore.addProof(requestID, ...Object.values(this.proof), JSON.stringify(cipher), [1], { from: account })
  return tx
}
...
contract('ProofStore', (accounts) => {
  ...
  describe('proofs', () => {
    it('successfully saves a proof', async () => {
      ...
      const output = await this.proofStore.getOutput(_requestID)
      const input = await this.proofStore.getPublicInput(_requestID)
      output.should.equal(JSON.stringify(cipher))
      input[0].should.be.bignumber.equal(1)
    })
  })
  ...
  describe('computation output', () => {
    it('should save a computation output', async () => {
      try {
