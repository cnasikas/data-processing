...
module.exports = class EthereumNode extends Node {
  ...
  printInfo () {
    ...
    console.log(logSymbols.info, `${utc}: Connected to blockchain node: ${host}`)
    console.log(logSymbols.info, `Default account: ${this.getDefaultAccount()}`)
  }
  ...
  notifyProcessor (requestID, encryptedKey) {
    return this._send(this.dataStoreInstance, 'notifyProcessor', requestID, encryptedKey)
  }

  async getEntity (address, key) {
    const res = await this._call(this.dataStoreInstance, key, address)
    const [name, pubkey] = res
    return [this.fromBytes(name), pubkey]
  }

  getProcessor (address) {
    return this.getEntity(address, 'processors')
  }

  getController (address) {
    return this.getEntity(address, 'controllers')
  }

  async getRequestInfo (requestID) {
    const res = await this._call(this.dataStoreInstance, 'requests', requestID)
    return [res[0], this.fromBytes(res[3]), res[4], res[1]]
  }

  async getDataSetInfo (dataSetID) {
    const res = await this._call(this.dataStoreInstance, 'dataStore', dataSetID)
    const [name, location, category, metadata, controller] = res
    return [this.fromBytes(name), location, this.fromBytes(category), metadata, controller]
  }

  async getCurrentProcessor () {
    const list = await this._call(this.dataStoreInstance, 'prList')
    return list[0]
  }

  addProof (requestID, proof, output, publicInput) {
    return this._send(this.proofStoreInstance, 'addProof', requestID, ...Object.values(proof), output, publicInput)
  }
}
