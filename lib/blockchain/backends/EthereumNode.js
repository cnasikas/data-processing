...
module.exports = class EthereumNode extends Node {
  ...
  getProcessor (address) {
    return this.getEntity(address, 'getProcessor')
  }

  getController (address) {
    return this.getEntity(address, 'getController')
  }

  async getRequestInfo (requestID) {
    const res = await this._call(this.dataStoreInstance, 'getRequestInfo', requestID)
    const [dataSetID, algorithmID, pubKey, requestor] = res
    return [dataSetID, this.fromBytes(algorithmID), pubKey, requestor]
  }

  async getDataSetInfo (dataSetID) {
    const res = await this._call(this.dataStoreInstance, 'getDataSetInfo', dataSetID)
    const [name, location, category, metaHash, controller] = res
    return [this.fromBytes(name), location, this.fromBytes(category), metaHash, controller]
  }

  getCurrentProcessor () {
    return this._call(this.dataStoreInstance, 'getCurrentProcessor')
  }
