...
module.exports = class EthereumNode extends Node {
  ...
  _exportContractFuntions (keys) {
    for (const func of keys) {
      this[func] = async (...args) => {
        const out = await this._send(func, ...args)
        return out
      }
    }
  }
  ...
  async _send (method, ...args) {
    ...
    if (!this.isMetaMask()) {
      options.gas = await this.contract[method].estimateGas(...args)
    }

    const tx = await new Promise((resolve, reject) => {
      this.contract[method](...args, options)
  ...
  registerDataSet (hash, name, location, category, metadata = '') {
    ...
    return this._send('registerDataSet', `0x${hash}`, name, location, category, metadata)
  }
  ...
  registerProcessor (name, pubkey) {
    ...
    name = this.toBytes(name)
    return this._send('registerProcessor', account, name, pubkey)
  }
  ...
  registerController (name, pubkey) {
    ...
    const account = this.getDefaultAccount()
    name = this.toBytes(name)
    return this._send('registerController', account, name, pubkey)
  }

  requestProcessing (datasetID, algorithmID, pubKey) {
    algorithmID = this.toBytes(algorithmID)
    return this._send('requestProcessing', `0x${datasetID}`, algorithmID, pubKey)
  }

  notifyProcessor (processorAddress, requestID, encryptedKey) {
    return this._send('notifyProcessor', processorAddress, requestID, encryptedKey)
  }

  getProcessor (address) {
    return this._call('getProcessor', address)
  }

  getRequestInfo (requestID) {
    return this._call('getRequestInfo', requestID)
  }

  getDataSetInfo (dataSetID) {
    return this._call('getDataSetInfo', dataSetID)
  }
}
